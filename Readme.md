...
2. **How to Call LLM**  
   ...
   - **Temperature** (0.0–2.0) and **top_p** (0.0–1.0) sliders in one row; defaults depend on the model (e.g. GPT/Gemini 0.7, LOCAL LLM 0.6 for temperature; top_p default 1.0).
...
OpenAI、たまは、GeminiのAPIキーを取得して環境変数に設定してください。あとは、配布パッケージのzipを解凍して、DokodemoLLMahk.exeを実行するだけ（解凍した他のファイルやフォルダはそのままにしてください）。トレイにアイコンが現れます。
...
2-2. プロンプトの入力  
`Win-Ctrl-o`でOpenAIのGPT、`Win-Ctrl-g`でGemini、**`Ctrl-Win-d`**でローカルLLMが呼び出されます。

UIは英語表示で、次の要素があります。
- **ドロップダウン**（履歴）と**複数行エディタ**（プロンプト入力）
- **Temperature**（0.0～2.0）と **top_p**（0.0～1.0）のスライダーが1行に並ぶ（モデルごとの既定値：GPT/Gemini は temperature 0.7、ローカルLLM は 0.6；top_p は既定 0.9）
...
1. **ビルド**  
   ...
   `build_exe.bat` はローカル一時フォルダでビルドして、最後に同じフォルダへ `DokodemoLLM.exe` をコピーします（Dropbox 等の同期フォルダでの WinError 110 回避のため）。  
   ※ spec では Qt/PyQt 系を除外しており、exe に Python や Qt のインストールは不要です。過去にビルドした exe で「Qt5WebEngineCore_conda.dll の解凍に失敗した」などのエラーが出る場合は、現在の spec で再ビルドしてください。
...
3. **常駐の起動**  
   AHK 起動時に `DokodemoLLM.exe serve` を自動で起動します（初回ショートカット時の待ち時間を減らすため）。  
   事前に手動で起動したい場合も同じく `DokodemoLLM.exe serve` を実行してください。  
   サーバー起動中（まだ ready 前）にショートカットを押しても、既存の `DokodemoLLM.exe` プロセスを検出した場合は二重起動しません。  
   ※この AHK ランチャーは Python 実行にはフォールバックせず、`DokodemoLLM.exe` のみを使用します。
...
毎回プロセスを立ち上げず、**`DokodemoLLM.exe serve` の常駐サーバーにHTTPで依頼**するため、応答が速くなります（起動・importは1回だけ）。

- **自動**: AHK 起動時に `DokodemoLLM.exe serve` をバックグラウンドで起動します。
- **再試行**: ショートカット呼び出し時に `health` がまだ通らない場合は、まず既存 `DokodemoLLM.exe` プロセスの有無を確認します。既に起動中ならそのまま ready 待機のみ行い、未起動のときだけ `DokodemoLLM.exe serve` を起動して1回だけ再試行します。
- **手動で常駐させたい場合**: あらかじめ次を実行しておくと、初回からすぐに依頼できます。  
  `DokodemoLLM.exe serve`  
  （ポート 28765 で待機。AHK はここにPOSTして結果を受け取ります。）
...
- 「^#d::」は、**Ctrl-Win-d**で起動：ローカルLLM（modelは環境変数`LOCALLLM_MODEL`で指定）

## ローカルLLM（DGX Spark）を使う場合（詳細）

DokodemoLLMはOpenAI互換APIをサポートしているため、NVIDIA DGX Sparkなどのマシン上で互換APIサーバーを起動すると、ローカルLLMとして利用できます。ここでは、vLLMをDGX Spark上でDockerコンテナとして動かす方法を例に説明します。8Bクラスのモデルでは、OpenAIやGeminiを呼び出すときの半分から1/3の時間で返ってきてキビキビとローカルLLMのメリットを享受できますが、70Bクラスのモデルではまったく逆の結果となりました。gpt-oss:120bなどほかのモデルでのレスポンス速度については試してみてください。
...
- **vLLM コンテナの起動**: DGX SparkにSSHなどでログインし、次を実行します。
  ...
  ```
- **起動確認**: `docker ps` および `docker logs -f vllm-llama` で確認。DGX上で `curl http://localhost:8000/v1/models` でモデル一覧（JSON）を取得。その `id` が環境変数 `LOCALLLM_MODEL` に設定する値です。このモデル一覧が表示される状態まで待ってから、DokodemoLLM.exeを起動してください。“curl: (56) Recv failure: 接続が相手からリセットされました”などのエラーが出ている間はひたすら待ちます。
...
- **環境変数**: `LOCALLLM_BASE_URL`（例: `http://192.168.xx.xxx:8000/v1` xx、xxxはDGXのIPアドレス）、`LOCALLLM_MODEL`（例: `nvidia/Llama-3.1-8B-Instruct-NVFP4`）、`LOCALLLM_API_KEY`（不要なら未指定、または`EMPTY`）。
